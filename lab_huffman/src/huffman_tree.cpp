                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(!singleQueue.empty()&&mergeQueue.empty()){
        TreeNode*next_node=singleQueue.front();
        singleQueue.pop();
        return next_node;
    }
    if(singleQueue.empty()&&!mergeQueue.empty()){
        TreeNode*next_node=mergeQueue.front();
        mergeQueue.pop();
        return next_node;
    }
    if(!mergeQueue.empty()&&!singleQueue.empty()){
        TreeNode*tempSingle = singleQueue.front();
        TreeNode*tempMerge = mergeQueue.front();
         if(tempSingle->freq.getFrequency()<tempMerge->freq.getFrequency()){
            singleQueue.pop();
            return tempSingle;
         }
         else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(size_t i = 0;i<frequencies.size();++i){
        TreeNode* newNode = new TreeNode(frequencies[i]);
        singleQueue.push(newNode);
     }
     while(singleQueue.size()+mergeQueue.size()>1){
        TreeNode*leftRemove = removeSmallest(singleQueue,mergeQueue);
        TreeNode*rightRemove = removeSmallest(singleQueue,mergeQueue);
        int internalFreq=leftRemove->freq.getFrequency()+rightRemove->freq.getFrequency();
        TreeNode*newInternal = new TreeNode(internalFreq);
        newInternal->left=leftRemove;
        newInternal->right=rightRemove;
        mergeQueue.push(newInternal);
     }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit()==true){
        ...
        TreeNode * newNode = new TreeNode(Frequency(0));
