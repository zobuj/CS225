...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(first_larger_idx<subroot->elements.size()){
        //checking if it is a valid index and if the key at the first larger idx is the key that we are looking for
        if(subroot->elements.at(first_larger_idx).key==key){
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /*------STEP 1------*/
    parent->children.insert(child_itr, new_right);//pointer to the new node that will be located in the children node. 
    /*------STEP 2------*/
    parent->elements.insert(elem_itr, *mid_elem_itr);//insert into the parents node, the child element that will be moving up that will be located at the median of the child node
    /*------STEP 3------*/
    //copy over the elements (and children) to the right of the child median into the new right node
    new_right->elements.assign(mid_elem_itr+1, child->elements.end());
    new_right->children.assign(mid_child_itr,child->children.end());
    /*------STEP 4------*/
    //make sure the left node only has the elements (and children) to the left of the child median
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(!subroot->is_leaf){
        ...
        BTreeNode*addNode = subroot->children.at(first_larger_idx);
        if(addNode->elements.size()==order){
