...
class KDTree
{
  ...
  private:
     ...
     */
     vector<Point<Dim>> permPoints;
     //Constructor Helper
    typename KDTree<Dim>::KDTreeNode * buildTree(int curDim,int low, int high);
    int partition(int low, int high, int curDim, int pivotIndex);
    void select(int low, int high, int curDim, int k);
    void copyHelper_(KDTreeNode * from, KDTreeNode * to);
    void remove_(KDTreeNode *curr);
    Point<Dim> findNearestNeighbor(KDTreeNode * curr,const Point<Dim> query, Point<Dim> best, int dim) const;
